...
const Navbar = () => {
  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5'>
      ...
      md:px-8 md:py-3 max-md:h-screen md:rounded-full backdrop-blur 
      bg-black/70 md:bg-white/10 md:border border-gray-300/20 overflow-hidden 
      transition-[width] duration-300 ${isopen ? 'max-md:w-full' : 'max-md:w-0'}`}>
        ...
        {favouriteMovies.length > 0 && <Link to='/favourite' onClick={() => { scrollTo(0, 0); setIsOpen(false) }}>Favorites</Link>}
      </div>
